...
<div class="max-w-xl mx-auto">
  <h2 class="text-xl font-bold mb-4">Tambah Data OS</h2>
  <div class="bg-white rounded-xl border p-6">
    <form method="POST" action="{{ route('os.store') }}">
      @csrf
      <div class="space-y-3">
        <div><label class="block text-sm font-medium mb-1">Nama *</label><input name="nama" required class="w-full border rounded-lg px-3 py-2 text-sm"></div>
        <div><label class="block text-sm font-medium mb-1">No WA</label><input name="no_wa" class="w-full border rounded-lg px-3 py-2 text-sm"></div>
        ...
        <div><label class="block text-sm font-medium mb-1">Interview Online</label>
          <select name="interview_online" class="w-full border rounded-lg px-3 py-2 text-sm">
            @foreach(['belum','hadir','tidak_hadir','reschedule'] as $s)<option value="{{ $s }}">{{ str_replace('_',' ',$s) }}</option>@endforeach
          </select>
        </div>
        ...
        <div><label class="block text-sm font-medium mb-1">Hasil</label>
          <select name="hasil" class="w-full border rounded-lg px-3 py-2 text-sm">
            @foreach(['belum','ok','ng','dipertimbangkan'] as $s)<option value="{{ $s }}">{{ $s }}</option>@endforeach
          </select>
        </div>
      </div>
